...
| **Safeguards** | Two independent secrets sent by separate channels: a 256-bit link verifier and a generated code, both compared in constant time. **Both are stored in readable form** (decision of 2026-09-12), so that a creator can retrieve a link after the one-time reveal; the accepted consequence is that anyone holding the database *or a backup of it* can use any link still inside its window, which turns a historical leak into live access in a way a hashed store would not. Weighed against the fact that this database already holds the position history in the clear, and that the codes are machine-generated rather than user-chosen, so no password-reuse risk arises. Retrieval requires the creator's session and `CanShare` on the device, and the secrets are excluded from the Art. 15 export, where they would leave the system in a downloadable file. A growing cooldown after wrong codes, and an IP rate limit outside it. Every read re-checks the window and the revocation flag, so withdrawal takes effect immediately. A "current position only" share discloses no route at all, and the API refuses any time range for it so the track cannot be reconstructed a fix at a time. The recipient never learns the device's identifier, only the creator's label. |
