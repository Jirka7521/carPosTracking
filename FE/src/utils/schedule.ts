...
// no reads of the clock except through an injectable `reference`.
// ---------------------------------------------------------------------------

import type { DeviceScheduleRuleDto } from '../services/apiTypes'

// Bit 0 is Sunday, matching both JavaScript's Date.getDay() and .NET's
// DayOfWeek — which is exactly why the API chose that numbering.
...
export function describeDaysMask(mask: number): string {
  ...
  for (let day = 0; day < 7; day++) {
    if (mask & (1 << day)) {
      days.push(DAY_LABELS[day])
    }
  }
  return days.length === 0 ? 'Never' : days.join(', ')
}
...
export function formatLocalDayTime(value: Date): string {
  return `${DAY_LABELS[value.getDay()]} ${formatMinuteOfDay(value.getHours() * 60 + value.getMinutes())}`
}
