...
export const FIRMWARE_PARAMETER_GROUPS: readonly FirmwareParameterGroup[] = [
  ...
  },
  {
    ...
    parameters: [
      ...
      { name: 'kAccelPeakEnabled', value: 'false', origin: 'fixed', meaning: 'Report the strongest per-axis reading of the reporting interval instead of one instantaneous sample. Costs nothing extra to send — the same one report per interval' },
      { name: 'kAccelSampleIntervalMs', value: '1000', origin: 'fixed', meaning: 'How often the sensor is sampled while peak tracking is on. The ADXL345 runs at 100 Hz, so 1000 ms sees one sample in a hundred; 100 ms catches far more' },
    ],
  },
  {
    title: 'Battery & temperature',
    parameters: [
      { name: 'kBatteryEnabled', value: 'true', origin: 'fixed', meaning: 'Report pack charge and modem die temperature. Owns the charge DETECTION; without it there is no "charging" sentinel' },
      ...
      { name: 'kBatteryReportFromMethods', value: 'true', origin: 'fixed', meaning: 'Publish one column of the battery method log as battery_pct. When false the older AT+CBC figure is sent instead' },
      { name: 'kBatteryReportSourceIndex', value: '3', origin: 'fixed', meaning: 'Which voltage source is published — 3 = the calibrated MEDIAN of the ADC burst, the log’s p4_* columns' },
      { name: 'kBatteryReportModelIndex', value: '1', origin: 'fixed', meaning: 'Which model scores it — 1 = the piecewise Li-ion curve, the log’s *_curve columns. Together: p4_curve' },
      { name: 'kUnplugFixTimeoutSeconds', value: '60', origin: 'fixed', meaning: 'Extra acquire budget when the charger comes off and that cycle found no position — the pack is invisible while charging, so the first real reading is worth chasing. 0 disables it' },
    ],
  },
  {
    title: 'Battery method log',
    parameters: [
      { name: 'kBatteryLogEnabled', value: 'true', origin: 'fixed', meaning: 'Measure the pack every way the board allows and write one CSV row per report to the card. One of its columns is what gets published' },
      { name: 'kBatteryVbatSensePin', value: '35', origin: 'fixed', meaning: 'Pack voltage sense — the SAME pin as the charge sense, read as a voltage here' },
      { name: 'kBatterySolarSensePin', value: '36', origin: 'fixed', meaning: 'Charge-input (solar/VIN) sense pin' },
      { name: 'kBatteryDividerRatio / kSolarDividerRatio', value: '2.0f / 2.0f', origin: 'fixed', meaning: 'On-board divider ratios; the solar one is an assumption that varies by board revision' },
      { name: 'kBatteryAdcSamples', value: '16', origin: 'fixed', meaning: 'ADC conversions per measurement — averaged AND medianed, which is two of the five voltage sources' },
      { name: 'kSolarInputThresholdMv', value: '1000', origin: 'fixed', meaning: 'Above this on the charge-input pin, a charge source is considered present' },
      { name: 'kBatteryNoReadingMv', value: '2000', origin: 'fixed', meaning: 'Below this the ADC path is logged as absent rather than as a flat pack — the USB case, where the sense pin is cut off from the cell' },
    ],
  },
  {
    title: 'WiFi',
  ...
  },
  {
    ...
    parameters: [
      ...
      { name: 'kBacklogFlushBudgetMs', value: '30000', origin: 'fixed', meaning: 'How long one flush may keep draining before handing the CPU back; it resumes on the next poll' },
      ...
      { name: 'kSdBatteryLogPath', value: '/sdcard/battery.csv', origin: 'fixed', meaning: 'The battery method log (plaintext — it holds no position data). A header mismatch rotates to battery2.csv … battery9.csv' },
      { name: 'kSdMaxBatteryLogRows', value: '20000', origin: 'fixed', meaning: 'Cap on the log’s data rows, header excluded; the oldest are dropped past this. 0 = no cap' },
      { name: 'kBootLogEnabled', value: 'true', origin: 'fixed', meaning: 'Record why the device restarted — reset reason, boot counter, and whether RTC memory survived' },
      { name: 'kSdBootLogPath', value: '/sdcard/boot.log', origin: 'fixed', meaning: 'One plaintext line per boot (restart forensics only, no position data)' },
      { name: 'kSdMaxBootLogLines', value: '200', origin: 'fixed', meaning: 'Cap on the boot log — far more history than any diagnosis needs' },
      { name: 'kBootLogPrintLines', value: '10', origin: 'fixed', meaning: 'How many previous boots to print to the console at start-up' },
    ],
  },
  {
    title: 'Deep sleep',
