...
.DESCRIPTION
    Bound to Ctrl+Shift+M (.vscode/tasks.json -> the "Dev Quick Menu" task). For
    each configuration value it shows the current value taken from the dev
    environment (or the previous run) and lets you keep it, type a new one, or —
    for the two crypto keys — generate a fresh one.
...
# Turn a SecureString from Read-Host into plain text (we must write it to .env).
...
# lower-cased option name (they are case-insensitive in Npgsql), so the dev
# connection string can supply the database values as keep-able current ones.
